...
namespace DotLiquid
{
    public class Context
    {
        ...
        private object Resolve(string key)
        {
            ...
            Match match = _singleQuotesRegex.Match(key);
            if (match.Success)
        ...
        private static object Liquidize(object obj)
        {
            ...
            var liquidTypeAttributes = obj.GetType().GetCustomAttributes(typeof(LiquidTypeAttribute), false);
            if (liquidTypeAttributes.Any())
            {
                var attr = (LiquidTypeAttribute)liquidTypeAttributes.First();
